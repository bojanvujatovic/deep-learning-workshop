...
model = L.Classifier(net.MnistMLP(784, n_units, 10)) # TODO here
...
for epoch in epochs:
    ...
    for i in six.moves.range(0, N, batchsize):
        x_batch = chainer.Variable(np.asarray(x_train[perm[i:i + batchsize]])) # TODO here
        y_batch = chainer.Variable(np.asarray(y_train[perm[i:i + batchsize]])) # TODO here
    ...
    for i in six.moves.range(0, N_test, batchsize):
        ...
        sum_loss += float(loss.data) * len(y_batch.data) # TODO here
        sum_accuracy += float(model.accuracy.data) * len(y_batch.data) # TODO here
